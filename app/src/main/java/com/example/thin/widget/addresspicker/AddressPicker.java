...
/**
 * Created by xingyan on 2018/1/5 10:04.
 * Email:lucky_xyic@sina.cn
 * ToDo:地址选择器（包括省级、地级、县级），地址数据见示例项目assets目录下。
 */
...
public class AddressPicker extends LinkagePicker {
    ...
    private String TAG = "AddressPicker==";

    public AddressPicker(Activity activity, List<AreaProvince> provinces) {
    ...
    private AreaProvince getSelectedProvince() {
        return provinces.get(selectedFirstIndex);
    }

    private AreaCity getSelectedCity() {
        return getSelectedProvince().getCities().get(selectedSecondIndex);
    }

    private AreaCounty getSelectedCounty() {
    ...
    public void onSubmit() {
        if (onLinkageListener != null) {
            AreaProvince province = getSelectedProvince();
            AreaCity city = getSelectedCity();
            AreaCounty county = null;
            if (!hideCounty) {
    ...
    public static class AddressProvider implements DataProvider {
        ...
        public AddressProvider(List<AreaProvince> provinces) {
            parseData(provinces);
        }
