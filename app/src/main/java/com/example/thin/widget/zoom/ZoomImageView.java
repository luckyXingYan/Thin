package com.example.thin.widget.zoom;
    ...
    public ZoomImageView(Context context, AttributeSet attrs, int defStyle) {
        ...
        mGestureDetector = new GestureDetector(context,
                new GestureDetector.SimpleOnGestureListener() {
                    ...
                    /**
                     * 同 onSingleTapUp ，但有附加条件，就是Android会确保单击之后短时间内没有再次单击，才会触发该函数。
                     *
                     * eg，如果监听双击事件：onSingleTapUp()会被触发两次。但是onSingleTapConfirmed()一次都不会被触发。
                     * 所以，如果你既想监听单击事件，又想监听双击时间，那么请使用onSingleTapConfirmed()函数。
                     * ===  单击行为，但这和onSingleTapUp不同，他不能再紧跟着发生单击行为，所以和onDoubleTap是互斥存在的
                     * @param e
                     * @return
                     */
                    @Override
